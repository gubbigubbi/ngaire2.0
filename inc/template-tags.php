...
if ( ! function_exists( 'nagaire_2_0_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function nagaire_2_0_posted_on() {
...
if ( ! function_exists( 'nagaire_2_0_entry_footer' ) ) :
/**
 ...
 * @return bool
 */
function nagaire_2_0_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'nagaire_2_0_categories' ) ) ) {
		...
		set_transient( 'nagaire_2_0_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so nagaire_2_0_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so nagaire_2_0_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in nagaire_2_0_categorized_blog.
 */
function nagaire_2_0_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'nagaire_2_0_categories' );
}
add_action( 'edit_category', 'nagaire_2_0_category_transient_flusher' );
add_action( 'save_post',     'nagaire_2_0_category_transient_flusher' );
